...
<div class="pipeline">
  ...
  <div class="pipeline-step"><span class="step-num">2</span><span class="step-label">Per-trial feature extraction</span><span class="step-detail">&nbsp;— 38 features × trial</span></div>
  <div class="pipeline-arrow">&#8595;</div>
  ...
  <div class="pipeline-arrow">&#8595;</div>
  <div class="pipeline-step"><span class="step-num">4</span><span class="step-label">Subject feature set</span><span class="step-detail">&nbsp;— 76 columns (38 + 38)</span></div>
  <div class="pipeline-arrow">&#8595;</div>
  <div class="pipeline-step"><span class="step-num">5</span><span class="step-label">Redundancy removal + quality join</span><span class="step-detail">&nbsp;— 57 remain + 2 quality </span></div>
</div>
```
:::
::: {.column width="42%"}
...
::: {.fragment}
- Median over trials: robust to 1 or 2 surviving borderline-quality trials
:::
::: {.fragment}
- IQR preserved: intra-subject instability may be informative
:::
:::
:::

::: notes
- Aggregation flow: 38 per-trial features × up to 10 valid trials per subject → median and IQR computed per feature per subject → 78 subject-level columns (39 medians + 39 IQRs) → redundancy removal leaves 57.
- Median aggregation: for each subject we take the median value of a feature across their valid trials. More robust than the mean when 1 or 2 trials are borderline quality.
...
- Strongest single-feature association: `pupil_relative_delta`, $\rho \approx 0.38$ with STAI_T 
...
![](../../output/plots/permutation_null_stai_t.png){width=80%}

::: {.fragment .callout style="text-align: center; display: block; margin-left: auto; margin-right: auto; width: fit-content;"}
...
![](../../output/plots/pca_colored.png){width=100% fig-align="center"}

::: {.columns style="margin-top: 16px; text-align: center; font-size: 0.8em;"}
::: {.column width="33%"}
### PC1 (21.8%)
**System Artifacts**  
...
::: {.columns style="margin-top: 20px; font-size: 0.9em;"}
...
Reflects overall resting stress levels, driven by the LC-NE pathway.
:::
...
- `pupil_relative_delta`
- `pupil_rebound`

Captures the magnitude of the reaction to relaxation, and the ability to return to baseline.
:::
...
- `pupil_nonlinearity`
- `pupil_early_late_diff`

Tracks competing autonomic drives (sympathetic vs parasympathetic) over the 30s window.
:::
:::

::: notes
- **Definitions for the audience/yourself:**
- **Tonic:** "Always-on" baseline state. High trait anxiety = a larger resting pupil.
- **Phasic:** Immediate, short-term reaction to a stimulus (the relaxation period). High anxiety limits the ability to recover back to baseline.
- **Temporal vs Phasic:** Phasic measures the *size/drop* of the reaction. Temporal measures the *shape* of the curve over time (e.g., struggling to adapt smoothly).
- **LC-NE:** Locus coeruleus-norepinephrine pathway. The brain's main alert/stress system, directly wired to pupil dilation. The more stressed you are generally, the higher the tone here.
:::

## PCA Loadings: Independent Support

<div style="width: 80%; margin: 0 auto; overflow: hidden; height: 360px; display: flex; align-items: flex-end;">
  <img src="../../output/plots/candidate_loadings.png" style="width: 100%; margin-bottom: -10px;">
</div>

::: {.columns style="margin-top: 15px; font-size: 0.85em; text-align: center;"}
::: {.column .fragment width="33%"}
### <i class="fa-solid fa-arrows-to-eye"></i> Convergent Validity
Same 6 candidates from univariate ranking dominate the two PCs.
:::

::: {.column .fragment width="33%"}
### <i class="fa-solid fa-code-branch"></i> Shared Variance
Loadings spread across PC2 and PC3, two supporting factors.
:::

::: {.column .fragment width="33%"}
### <i class="fa-solid fa-check-double"></i> Corroboration
...
```{=html}
<div style="display: flex; flex-direction: column; align-items: stretch; gap: 8px; font-size: 0.65em; margin: 10px 0;">
  
  <!-- Top Layer: Interfaces & Config -->
  <div style="display: flex; justify-content: center; gap: 15px;">
    <div style="background: #f8f9fa; border: 1px solid #ddd; padding: 8px 14px; border-radius: 8px; color: #2d3748; font-weight: 600;">
      <i class="fa-solid fa-terminal" style="color:#6b7a8d;"></i> <code>main.py</code>
    </div>
    <div style="background: #f8f9fa; border: 1px solid #ddd; padding: 8px 14px; border-radius: 8px; color: #2d3748; font-weight: 600;">
      <i class="fa-solid fa-book" style="color:#6b7a8d;"></i> <code>analysis.ipynb</code>
    </div>
    <div style="background: #f8f9fa; border: 1px solid #ddd; padding: 8px 14px; border-radius: 8px; color: #2d3748; font-weight: 600;">
      <i class="fa-solid fa-sliders" style="color:#6b7a8d;"></i> <code>constants.py</code>
    </div>
  </div>

  <div style="text-align: center; color: var(--accent); font-size: 1.4em; line-height: 0.4; margin-bottom: 8px;">&#8595;</div>

  <!-- Middle Layer: Preprocessing (Teal) -->
  <div style="background: rgba(27, 107, 115, 0.04); border: 2px solid var(--accent); border-radius: 10px; padding: 12px 16px; margin: 0 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center;">
      <div>
        <div style="font-weight: 700; color: var(--accent); font-size: 1.1em; margin-bottom: 2px;">
          <i class="fa-solid fa-filter"></i> Phase 1: Preprocessing Pipeline
        </div>
        <div style="color: #4a5568; margin-bottom: 6px; font-family: monospace; font-size: 0.9em;">pipeline.py &rarr; run_preprocessing()</div>
        <div style="display: flex; gap: 6px; font-size: 0.85em;">
          <span style="background:#fff; border:1px solid #cbd5e0; padding:2px 6px; border-radius:4px;"><i class="fa-solid fa-check-circle" style="color:var(--accent);"></i> validation.py</span>
          <span style="background:#fff; border:1px solid #cbd5e0; padding:2px 6px; border-radius:4px;"><i class="fa-solid fa-wave-square" style="color:var(--accent);"></i> features.py</span>
        </div>
      </div>
      <div style="text-align: right; background: #fff; padding: 6px 10px; border-radius: 6px; border: 1px dashed var(--accent);">
        <div style="font-weight: 700; color: var(--accent); font-size: 0.85em; margin-bottom: 2px;">Outputs</div>
        <div style="color: #2d3748; font-family: monospace; font-size: 0.85em;">features.parquet</div>
        <div style="color: #2d3748; font-family: monospace; font-size: 0.85em;">subjects.parquet</div>
        <div style="color: #718096; font-size: 0.75em; margin-top: 3px;"><i class="fa-solid fa-file-csv"></i> Doc: feature_catalog</div>
      </div>
    </div>
  </div>

  <div style="text-align: center; color: #805ad5; font-size: 1.4em; line-height: 0.4; margin-bottom: 8px;">&#8595;</div>

  <!-- Bottom Layer: Analysis (Purple) -->
  <div style="background: rgba(128, 90, 213, 0.04); border: 2px solid #805ad5; border-radius: 10px; padding: 12px 16px; margin: 0 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center;">
      <div>
        <div style="font-weight: 700; color: #805ad5; font-size: 1.1em; margin-bottom: 2px;">
          <i class="fa-solid fa-chart-line"></i> Phase 2: Analysis Pipeline
        </div>
        <div style="color: #4a5568; margin-bottom: 6px; font-family: monospace; font-size: 0.9em;">pipeline.py &rarr; run_analysis()</div>
        <div style="display: flex; gap: 6px; font-size: 0.85em;">
          <span style="background:#fff; border:1px solid #cbd5e0; padding:2px 6px; border-radius:4px;"><i class="fa-solid fa-object-group" style="color:#805ad5;"></i> selection.py</span>
          <span style="background:#fff; border:1px solid #cbd5e0; padding:2px 6px; border-radius:4px;"><i class="fa-solid fa-chart-area" style="color:#805ad5;"></i> plotting.py</span>
        </div>
      </div>
      <div style="text-align: right; background: #fff; padding: 6px 10px; border-radius: 6px; border: 1px dashed #805ad5;">
        <div style="font-weight: 700; color: #805ad5; font-size: 0.85em; margin-bottom: 2px;">Outputs</div>
        <div style="color: #2d3748; font-family: monospace; font-size: 0.85em;">zscore_params.csv</div>
        <div style="color: #2d3748; font-family: monospace; font-size: 0.85em;">loadings_summary.csv</div>
        <div style="color: #718096; font-size: 0.75em; margin-top: 3px;"><i class="fa-solid fa-image"></i> Diagnostic Plots</div>
      </div>
    </div>
  </div>
...
::: notes
- The pipeline is explicitly decoupled into two phases, preparing it for production scaling.
- Phase 1 (Preprocessing) isolates feature extraction. It generates the core parquet datasets, which scales horizontally per user session.
- Phase 2 (Analysis) standardizes the data and generates our baseline metrics (z-scores, PCA weights).
- The feature catalog acts as living documentation, while `constants.py` centralizes all tunables.
:::

## Deployment Considerations

```{=html}
<div style="display: flex; flex-direction: column; align-items: stretch; gap: 8px; font-size: 0.65em; margin: 10px auto; max-width: 800px;">

  <!-- Data Arrival & Storage Phase (Neutral) -->
  <div class="fragment" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
    <div style="background: #f8f9fa; border: 1px solid #ddd; padding: 10px 14px; border-radius: 8px;">
      <div style="font-weight: 700; color: #2d3748; margin-bottom: 4px; font-size: 1.05em;"><i class="fa-solid fa-cloud-arrow-up"></i> Data Arrival</div>
      <div style="color: #4a5568; line-height: 1.3;">Receive new VR session data and validate structure.</div>
      <div style="margin-top: 8px;"><span class="tool-tag" style="background:#e2e8f0; color:#4a5568; border-color:#cbd5e0;">Pydantic</span></div>
    </div>
    <div style="background: #f8f9fa; border: 1px solid #ddd; padding: 10px 14px; border-radius: 8px;">
      <div style="font-weight: 700; color: #2d3748; margin-bottom: 4px; font-size: 1.05em;"><i class="fa-solid fa-database"></i> Secure Storage</div>
      <div style="color: #4a5568; line-height: 1.3;">Save raw timeseries and version datasets.</div>
      <div style="margin-top: 8px;"><span class="tool-tag" style="background:#e2e8f0; color:#4a5568; border-color:#cbd5e0;">Google Cloud</span><span class="tool-tag" style="background:#e2e8f0; color:#4a5568; border-color:#cbd5e0;">DVC</span></div>
    </div>
  </div>

  <!-- Arrow + Extraction Phase (grouped fragment) -->
  <div class="fragment">
    <div style="text-align: center; color: var(--accent); font-size: 1.2em; line-height: 0.5; margin-bottom: 8px;">&#8595;</div>
    <!-- Extraction Phase (Teal - matches Phase 1) -->
    <div style="background: rgba(27, 107, 115, 0.04); border: 2px solid var(--accent); border-radius: 10px; padding: 12px 16px;">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
          <div style="font-weight: 700; color: var(--accent); font-size: 1.1em; margin-bottom: 2px;"><i class="fa-solid fa-gears"></i> Feature Extraction</div>
          <div style="color: #4a5568;">Run the exact same Python pipeline to extract metrics.</div>
        </div>
        <div style="text-align: right;">
          <span class="tool-tag">Docker</span><span class="tool-tag">src/pipeline.py</span>
        </div>
      </div>
    </div>
  </div>

  <!-- Arrow + Scoring & Delivery Phase (grouped fragment) -->
  <div class="fragment">
    <div style="text-align: center; color: #805ad5; font-size: 1.2em; line-height: 0.5; margin-bottom: 8px;">&#8595;</div>
    <!-- Scoring & Delivery Phase (Purple - matches Phase 2) -->
    <div style="background: rgba(128, 90, 213, 0.04); border: 2px solid #805ad5; border-radius: 10px; padding: 12px 16px;">
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
         <div>
           <div style="font-weight: 700; color: #805ad5; font-size: 1.1em; margin-bottom: 2px;"><i class="fa-solid fa-chart-bar"></i> Inference &amp; Scoring</div>
           <div style="color: #4a5568; margin-bottom: 8px;">Apply frozen training Z-scores &amp; PCA loadings to new trials.</div>
           <span class="tool-tag" style="color:#805ad5; background:rgba(128,90,213,0.1); border-color:rgba(128,90,213,0.2);">MLflow</span>
           <span class="tool-tag" style="color:#805ad5; background:rgba(128,90,213,0.1); border-color:rgba(128,90,213,0.2);">scikit-learn</span>
         </div>
         <div style="border-left: 1px dashed rgba(128, 90, 213, 0.3); padding-left: 15px;">
           <div style="font-weight: 700; color: #805ad5; font-size: 1.1em; margin-bottom: 2px;"><i class="fa-solid fa-server"></i> Results Delivered</div>
           <div style="color: #4a5568; margin-bottom: 8px;">Return final biomarker scores via API.</div>
           <span class="tool-tag" style="color:#805ad5; background:rgba(128,90,213,0.1); border-color:rgba(128,90,213,0.2);">FastAPI</span>
           <span class="tool-tag" style="color:#805ad5; background:rgba(128,90,213,0.1); border-color:rgba(128,90,213,0.2);">Postman</span>
         </div>
      </div>
    </div>
  </div>

  <!-- Arrow + Monitoring Phase (grouped fragment) -->
  <div class="fragment">
    <div style="text-align: center; color: #6b7a8d; font-size: 1.2em; line-height: 0.5; margin-bottom: 8px;">&#8595;</div>
    <!-- Monitoring Phase (Dark Grey) -->
    <div style="background: #f8f9fa; border: 2px solid #6b7a8d; border-radius: 10px; padding: 12px 16px;">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
          <div style="font-weight: 700; color: #4a5568; font-size: 1.1em; margin-bottom: 2px;"><i class="fa-solid fa-bell"></i> Model Monitoring</div>
          <div style="color: #718096;">Track feature drift and trigger pipeline tests on schedule.</div>
        </div>
        <div style="text-align: right;">
          <span class="tool-tag" style="color:#4a5568; background:#e2e8f0; border-color:#cbd5e0;">GitHub Actions</span>
          <span class="tool-tag" style="color:#4a5568; background:#e2e8f0; border-color:#cbd5e0;">Cloud Logging</span>
        </div>
      </div>
    </div>
  </div>
...
::: notes
- I want to highlight the stack I actually work with to bridge the gap to production.
- Pydantic ensures the payloads I get match what my pipeline expects.
- DVC versions my datasets on Google Cloud so my experiments are reproducible.
- I can use Docker to containerize the exact environment my code needs.
- MLflow logs my models and parameters, keeping my registry clean.
- I test the FastAPI endpoints with Postman, and rely on GitHub actions for automated CI/CD checks.
- I don't pretend to be the infra engineer, but I know how to package my work so they can use it.
:::
